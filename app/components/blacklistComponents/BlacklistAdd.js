...
export default class BlacklistAdd extends Component {
	...
	handleAdd = () => {
		...
		else if (!this.state.name) this.props.updateStatus('Name is required');
		else if (!this.state.url) this.props.updateStatus('Url is required');
		...
		return (
			<tr>
				...
				<td>
					...
					>
						<FontAwesome
							name={this.state.open ? 'check' : 'plus'}
							style={{ color: '#337ab7' }}
							size="2x"
						/>
					</button>
				...
				<td className="page">
					...
					<Collapse in={this.state.open}>
						<div>{this.state.open ? this.renderInput() : null}</div>
					</Collapse>
				</td>
			</tr>
		);
